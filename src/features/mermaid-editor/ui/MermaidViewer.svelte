<script lang="ts">
  ...
  // Control de concurrencia para renderizado asíncrono
  ...
  onMount(async () => {
    try {
      ...
      error = 'Error al cargar Mermaid.js: ' + String(e);
    }
  });

  $effect(() => {
    const rawCode = content;
    if (isReady && rawCode && rawCode.trim()) {
      const requestId = ++currentRequestId;

      (async () => {
        try {
          const valRes = await validateMermaidJs(rawCode);
          if (requestId !== currentRequestId) return;

          if (!valRes.valid) {
            error = valRes.error || 'Error de sintaxis en diagrama Mermaid';
            return;
          }

          const res = await renderMermaidJsSvg(rawCode, 'mermaidjs-viewer');
          if (requestId !== currentRequestId) return;
          ...
          const svgStr = res.svg || '';
          error = '';
          scale = 1;
          panX = 0;
          panY = 0;

          // Extraer viewBox inicial
          ...
          if (!parsedVb) {
            const widthMatch = svgStr.match(/width=["']([0-9.]+)(?:px)?["']/i);
            const heightMatch = svgStr.match(/height=["']([0-9.]+)(?:px)?["']/i);
            const w = widthMatch ? parseFloat(widthMatch[1]) : 800;
            const h = heightMatch ? parseFloat(heightMatch[1]) : 600;
            parsedVb = { x: 0, y: 0, w: Math.max(w, 100), h: Math.max(h, 100) };
          }
    ...
    } else if (isReady && (!rawCode || !rawCode.trim())) {
      svgContent = '';
      error = '';
      baseViewBox = null;
    }
...
<div class="mermaid-container" class:split-mode={!readOnly}>
  {#if !readOnly}
    <div class="editor-pane">
      <div class="editor-header-bar">
        <div class="editor-title-group">
          <span>Código Mermaid</span>
          <span class="engine-badge" title="Visualizador oficial de Mermaid.js">Mermaid.js</span>
        </div>
  ...
  >
    {#if error}
      <div class="error-msg">
        <div class="error-title">Error al renderizar diagrama Mermaid (Mermaid.js)</div>
        <pre>{error}</pre>
      </div>
    {/if}
    {#if !isReady}
      <div class="loading">Cargando Mermaid.js...</div>
    {/if}
    {#if svgContent && !error}
