<script lang="ts">
  ...
  async function initMilkdown(initialText: string) {
    ...
    try {
      ...
      // Sincronizar si el contenido cambió durante la inicialización
  ...
  $effect(() => {
    ...
    // Inicializar si el nodo DOM ya está montado y el editor aún no se ha creado
    if (containerRef && !editorInstance && !isInitializing && activeMarkdown) {
      initMilkdown(nextContent);
      return;
    }

    // Actualizar contenido del editor existente si cambió la nota
